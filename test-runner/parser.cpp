...
std::string createResultItem(const TestMetadata& metadata, bool hasFailedTests) {
    ...
    if (metadata.status != "errored") {
        ...
        html.append(" src=\"data:image/png;base64," + base64::encode(metadata.actual.c_str(), metadata.actual.size()) + "\"");
        html.append(" data-alt-src=\"data:image/png;base64," + base64::encode(metadata.expected.c_str(), metadata.expected.size()) + "\">\n");

        html.append("<img width=" + mbgl::util::toString(metadata.size.width));
        html.append(" height=" + mbgl::util::toString(metadata.size.height));
        html.append(" src=\"data:image/png;base64," + base64::encode(metadata.diff.c_str(), metadata.diff.size()) + "\">\n");
    } else {
