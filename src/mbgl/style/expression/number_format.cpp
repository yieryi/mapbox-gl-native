...
{
      if (locale_) {
            locale = std::unique_ptr<Expression>(std::move(*locale_));
      }
      if (currency_) {
            currency = std::unique_ptr<Expression>(std::move(*currency_));
      }
      if (minFractionDigits_) {
            minFractionDigits = std::unique_ptr<Expression>(std::move(*minFractionDigits_));
      }
      if (maxFractionDigits_) {
            maxFractionDigits = std::unique_ptr<Expression>(std::move(*maxFractionDigits_));
      }
}
...
ParseResult NumberFormat::parse(const Convertible& value, ParsingContext& ctx) {
    ...
    ParseResult number = ctx.parse(arrayMember(value, 1), 1, {type::Number});
    if (!number) {
        ctx.error("Failed to parse the number.");
        return ParseResult();
    }

    type::Type type = (*number)->getType();
    ...
    if (currencyOption) {
        currency = ctx.parse(*currencyOption, 1, {type::String});
        if (!currency) {
    ...
    ParseResult minFractionDigits;
    if (minFractionDigitsOption) {
        minFractionDigits = ctx.parse(*minFractionDigitsOption, 1, {type::Number});
        if (!minFractionDigits) {
    ...
    ParseResult maxFractionDigits;
    if (maxFractionDigitsOption) {
        maxFractionDigits = ctx.parse(*maxFractionDigitsOption, 1, {type::Number});
        if (!maxFractionDigits) {
    ...
    return ParseResult(std::make_unique<NumberFormat>(std::move(*number),
                                                      std::move(locale),
                                                      std::move(currency),
                                                      std::move(minFractionDigits),
                                                      std::move(maxFractionDigits)));
}
