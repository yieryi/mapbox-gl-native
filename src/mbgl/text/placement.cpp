    ...
    auto placeSymbol = [&] (SymbolInstance& symbolInstance) {
        ...
        if (horizontalTextIndex) {
            ...
            const auto updatePreviousOrientationIfNotPlaced = [&](bool isPlaced) {
                    style::TextPlacementModeType previousOrientation = style::TextPlacementModeType::Horizontal;
            ...
            const auto placeTextForPlacementModes = [&] (auto& placeHorizontalFn, auto& placeVerticalFn) {
                if (bucket.allowVerticalPlacement && symbolInstance.writingModes & WritingModeType::Vertical) {
                    ...
                    for (auto& placementMode : bucket.placementModes) {
                        if (placementMode == style::TextPlacementModeType::Vertical) {
            ...
            if (variableTextAnchors.empty()) {
                const auto placeFeature = [&] (CollisionFeature& collisionFeature, style::TextPlacementModeType orientation) {
                ...
                const auto placeHorizontal = [&] {
                    return placeFeature(symbolInstance.textCollisionFeature, style::TextPlacementModeType::Horizontal);
                };

                const auto placeVertical = [&] {
                    if (bucket.allowVerticalPlacement && symbolInstance.verticalTextCollisionFeature) {
                        return placeFeature(*symbolInstance.verticalTextCollisionFeature, style::TextPlacementModeType::Vertical);
                    }
                ...
                const auto placeFeatureForVariableAnchors = [&] (CollisionFeature& collisionFeature, style::TextPlacementModeType orientation) {
                ...
                const auto placeHorizontal = [&] {
                    return placeFeatureForVariableAnchors(symbolInstance.textCollisionFeature, style::TextPlacementModeType::Horizontal);
                };

                const auto placeVertical = [&] {
                    if (bucket.allowVerticalPlacement && !placed.first && symbolInstance.verticalTextCollisionFeature) {
                        return placeFeatureForVariableAnchors(*symbolInstance.verticalTextCollisionFeature, style::TextPlacementModeType::Vertical);
                    }
                    return std::pair<bool, bool>{false, false};
                };
                ...
                style::TextPlacementModeType previousOrientation = updatePreviousOrientationIfNotPlaced(placed.first);
...
void Placement::markUsedJustification(SymbolBucket& bucket,
                                      ...
                                      style::TextPlacementModeType orientation) {
    ...
    const auto justificationToIndex = [&symbolInstance, orientation] (style::TextJustifyType type) -> optional<std::size_t> {
        ...
        if (orientation == style::TextPlacementModeType::Vertical) {
...
void Placement::markUsedOrientation(SymbolBucket& bucket, style::TextPlacementModeType orientation, SymbolInstance& symbolInstance) {
    auto horizontal = orientation == style::TextPlacementModeType::Horizontal ?
                                     optional<style::TextPlacementModeType>(orientation) : nullopt;
    auto vertical = orientation == style::TextPlacementModeType::Vertical ?
                                     optional<style::TextPlacementModeType>(orientation) : nullopt;
